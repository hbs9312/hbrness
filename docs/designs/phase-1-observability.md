...
#### 상관 관계
- correlation_header: `X-Request-Id` (없으면 middleware 가 생성)
- error_code_to_log_tag: true (Phase 1 (1) ErrorCode 자동 주입)
```
...
| 상관 관계 | optional | header 명·error_code 주입 여부 |
...
map-tasks
   │
   ├── impl-schema → impl-repositories → impl-error-codes → impl-services → impl-controllers
   │                                                                              │
   │                                                                              ▼
   ├── impl-middleware  ← impl-controllers 후
   │
   └── impl-observability  ← impl-error-codes 후, impl-middleware 와 병렬 가능
         ↓ (consumed by)
       impl-services (logger), impl-middleware (tracing middleware 추가),
       generate-tests (assertion: required_tags 가 로그에 존재)
```
...
- **Custom business metrics** — 로그인 성공률 등. impl-services 가 OTel API 로 직접 instrument. Phase 2 에서 별도 가이드.
...
- **PII / GDPR** — `redact` 기본 5종(password / token / ssn / card_number / email?) 외 도메인별 추가는 사용자 선언. `backend.md.observability.redact_extra: ["dob", "phone"]` 같은 확장 검토.
...
- [ ] `specflow:validate` ts-rules 33~37 추가 (warning)
- [ ] `backflow:impl-observability` 실행 시 4종 파일 + `.env.example` 갱신
- [ ] `backend.md.observability.*` 신규 키 추가
...
- [ ] `backflow/CONTRACTS.md` 갱신
- [ ] `impl-middleware` AppExceptionFilter 가 tagError 호출하도록 housekeeping
- [ ] 최소 1개 실프로젝트(이 레포 또는 velvetalk)에서 E2E 적용 — TS 작성 → impl 실행 → 실 로그 출력에서 `trace_id` / `error_code` 확인
