...
### error_shown 호출 위치 — 단일 지점 원칙 (XR-005)

error_shown 은 **사용자에게 실제로 노출된 에러** 만 기록해야 한다 (silent / 자동 retry 성공한 에러는 제외). 같은 에러가 여러 layer 에서 중복 track 되지 않도록 다음 우선순위로 **한 곳만** 선택:

1. **presentError wrapper** (있으면 최우선): handleError decision 을 받아 실제 UI 렌더(toast/modal/redirect) 직전. inline binding 도 form.setError 호출 직전. 모든 노출 경로가 이 wrapper 를 통과하므로 단일 지점 보장
2. **inline 바인딩 + presentError 미사용 프로젝트**: mutation onError 안에서 `decision.uiFlow !== 'silent'` 일 때만 호출
3. **response interceptor**: 1, 2 가 모두 부재한 단순 프로젝트에서만. uiFlow 'silent' / retriable=true 자동 retry 성공 케이스는 호출 X

**금지**: 위 3곳에 동시 호출. 한 곳만 선택. validate-code §10.x (Phase 2) 가 중복 검출 룰 추가 예정.
