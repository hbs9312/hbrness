...
- **backflow**: `scan-codebase → map-tasks → impl-schema → impl-repositories → impl-services → impl-controllers → impl-middleware → impl-integrations → generate-tests` + `validate-{code,api,security,tests}`
...
| `validate-security` | backflow | 모든 외부 공개 서비스 (Phase 2 첫 항목 구현 완료) |
...
**Phase 2 status (2026-04-28)**: `backflow:validate-security` 구현 완료. 설계 문서(`docs/designs/phase-2-validate-security.md`), dispatcher skill, cleanroom validator agent, `backend.md.security_validation` scope/policy block, `CONTRACTS.md` canonical flow 반영.
