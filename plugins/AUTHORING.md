...
## 3-Tier Storage Convention

### 결정 플로우

```
이 데이터가 사용자의 여러 코딩 에이전트에서 공유되어야 하는가?
│
├── 예 (또는 "상관없음") ──────────────▶ Tier 1 (tool-agnostic)
│
├── 아니오, 하네스 고유 시스템(Claude의 자동 메모리, codex의 AGENTS.md 등)과
│   동일 디렉토리에 살아야 함 ────────▶ Tier 2 (harness-placeholder)
│
└── 이 스킬 자체가 특정 하네스 고유 기능에 종속되어
    다른 하네스에서는 의미가 없음 ─────▶ Tier 3 (harness-gated)
```

**의심스러우면 Tier 1.** 대부분의 스킬은 Tier 1이 맞습니다.
...
대신 Tier 1 (`~/.hbrness/…`) 또는 Tier 2 placeholder (`{HARNESS_HOME}/…`) 를 사용하거나, Tier 3로 게이팅하라.

## 체크리스트 — 신규 스킬 작성 시

- [ ] 저장 경로가 있다면 Tier 1/2/3 중 **어느 쪽인지 의식적으로 결정**
