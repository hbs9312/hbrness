...
**분포**: ✅{N} · ⚠️{N} · ❌{N} · 🤔{N}  |  **미해결 스레드**: {N}건
**처리 순서 제안**: `{공백으로 구분된 번호 나열, Valid → Partial → Unclear → Invalid 순}`
...
어떤 번호부터 처리할까요?
`"1번 처리"` · `"Valid 전부 처리"` · `"3번 뭐 확인?"` · `"4번 반박 초안"`
````
...
- **타당성 평가 원칙**:
  ...
  - 권장 대응은 다섯 가지 중 선택: **수정 / 검토 / 논의 / 반박 / 무시** — Valid는 수정, Partial은 검토, Unclear는 논의, Invalid는 반박 또는 무시에 느슨하게 대응.
- **대시보드 테이블 정렬**:
  - 기본은 전역 번호(`#`) 오름차순
  - `처리 순서 제안`은 Valid → Partial → Unclear → Invalid 순, 각 그룹 내 번호순
