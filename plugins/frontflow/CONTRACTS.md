...
impl-interactions         ← FS BR + UI 인터랙션 + WF 상태 매트릭스 + TS 상태 enum
     │
     ▼
impl-api-integration      ← TS §API 설계 (F5 stub → 실제 호출 교체)
     │
     ▼
...
> **Phase 1 로드맵 영향**: `docs/plugin-gaps-and-plan.md` §3.3 (3) 에서 `impl-api-integration` 이 `backflow:export-api-contract` + `frontflow:sync-api-client` 로 분리 예정. §3.7 의 FS "이벤트 트래킹 테이블" 이 추가되면 신설 `impl-tracking` 행이 이 표에 추가된다.
