...
**금지 영역**: `components/` / `pages/` / `hooks/` / `api/` 및 공통 tracking 파일(`track.ts`, `events.ts`, `index.ts`, `selected-adapter.ts`)
- gtag / amplitude / mixpanel / posthog / dd-trace / Sentry 등 벤더 식별자 금지
- `track()` API 사용

**예외**: `adapters/{vendor}.ts` 파일 내부는 벤더 식별자 허용
